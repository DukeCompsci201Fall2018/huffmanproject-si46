...
public class HuffProcessor {
	...
	private void writeCompressedBits(String[] encodings, BitInputStream in, BitOutputStream out) {
		...
		String fin = encodings[PSEUDO_EOF];
		out.writeBits(fin.length(), Integer.parseInt(fin, 2));

		/*
		 * if (val==-1) { String code= encodings[PSEUDO_EOF];
		 * out.writeBits(code.length(), Integer.parseInt(code,2)); } else { String code=
		 * encodings[val]; out.writeBits(code.length(), Integer.parseInt(code,2));
		 * 
		 * }
		 */

		/*
		 * String code=encodings[val]; out.writeBits(code.length(),
		 * Integer.parseInt(code,2));
		 * 
		 * code=encodings[PSEUDO_EOF]; out.writeBits(code.length(),
		 * Integer.parseInt(code,2));
		 */
	}
	...
	public void decompress(BitInputStream in, BitOutputStream out) {
		...
		// make sure file is Huffman decoded by checking that first 32 bits are the
		// value HUFF_TREE
		...
		// traverse root-to-leaf paths, writing leaf values to the output file. STOP
		// when finding PSEUDO_EOF
